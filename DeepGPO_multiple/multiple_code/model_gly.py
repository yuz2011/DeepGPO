...
def simlarcalc(spectrum_1_intensity,spectrum_2_intensity,type): 
    ...
    if type=="cos_cbrt":
        cos = torch.nn.CosineSimilarity(dim=1)
        sim = cos(torch.pow(torch.clamp(spectrum_1_intensity, min=0), 1/3),torch.pow(torch.clamp(spectrum_2_intensity, min=0), 1/3))
    if type=="corre_sqrt":
...
class Metric_byBY_outputmsms(MetricBase):
    ...
    def __init__(self,savename, pred=None, target=None, 
        ...
        self.id_list=[]
        self.nj=0
        self.repsequence=pd.DataFrame(columns=['repsequence',"idecoration",'charge',
                                               "ipeptide","iPlausibleStruct",
