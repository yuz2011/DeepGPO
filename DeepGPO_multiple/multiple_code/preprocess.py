...
class PPeptideLoaderbyBY(Loader):
    ...
    def _load(self, fpath: str):
        fields = ["peptide", "charge", "ions_by","ions_BY","decoration","decoration_ACE","PlausibleStruct",'irt',"_id","GlySpec","iden_pep","weights"]
        use_weights=False
        ...
        return DataSet(datadict),use_weights
    def load(self, paths: Union[str, Dict[str, str]] = None) -> DataBundle:
        ...
        use_weights=datasets["train"][1]
        datasets["train"]=datasets["train"][0]
        data_bundle = DataBundle(datasets=datasets)

        return data_bundle,use_weights

class PPeptidePipebyBY(Pipe):
    def __init__(self,vocab=None,args=None):
        self.fields=["peptide", "charge", "ions_by","ions_BY","decoration","decoration_ACE","PlausibleStruct",'irt',"_id","GlySpec","iden_pep","weights"]
        self.vocab=vocab
        self.args=args
    def _tokenize(self, data_bundle):
        for name, dataset in data_bundle.datasets.items():
    ...
    def process(self, data_bundle: DataBundle,use_weights) -> DataBundle:
        if False:
            pass
        ...
        def strct_token(instance):
            PlausibleStruct = instance["PlausibleStruct"]
            struct_list = PlausibleStruct.split(";")
            graph_list = []
        ...
        def count_edge(instance):
            # PlausibleStruct=instance["PlausibleStruct"].split(";")
            # edge_list=[]
            # for st in PlausibleStruct:
            #     edge_list.append(st.count("("))
            return {"graph_edges":instance["PlausibleStruct"].count("(")}
        ...
        if use_weights:
            data_bundle.set_target('irt',"weights")
        else:
    ...
    def process_from_file(self, paths=None) -> DataBundle:
        data_bundle,use_weights = PPeptideLoaderbyBY().load(paths)
        return self.process(data_bundle,use_weights)
    
# from utils import *
# fpath="/remote-home1/yxwang/test/zzb/DeepGlyco/DeepSweet_v1/code2/task_processing/NO/multiple/multiple_data/multiple_test_data_1st_byBYprocessed.json"   #输入数据什么格式
